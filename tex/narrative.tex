...
In this previous work we have modeled with RANS and momentum sources an SMR core comprised of 37 assemblies (each 17$\times 17$), inspired by the Nuscale core. Each assembly has an active fuel length of $2$ m for which the the power distribution is obtained with a precursor Monte Carlo neutron transport calculation. The Reynolds number based on the rod diameter and the bulk velocity is $Re=50000$ and the Prandtl number is $Pr=1$.
