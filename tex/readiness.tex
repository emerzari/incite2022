...
Numerical discretization of (1)--(3)
=======
\vspace{-.25in}
...
of Busco et. al. \cite{busco2019invariant} for a 5x5 bundle and extrapolating
to 5 17x17 axial grids or roughly $60 x$ larger.
...
\noindent{\em State-of-the-Art}
The numerical discretization of the Navier-Stokes and thermal-transport
equations in Nek5000/RS is based on the spectral element method (SEM)
\cite{pat84}, which is a high-order weighted residual formulation similar to
the finite element method (FEM).  The SEM derives its computational efficiency
from matrix-free operator evaluations that allow the matrix-vector products
required for iterative solvers to be expressed as fast
...
matrix, $\Dh_{ip} := \left. \dd{l_p}{\xi} \right|_{\xi_i}$.   In typical
...
=======
>>>>>>> 65fc295c3fa13d9ef1c93700f65d53894faeba0e
...
typically requires only 0.1--0.4 seconds per step on Summit,  as illustrated
...
Table~\ref{dns-strong} demonstrates performance for {\bf DNS $5\times5$} on
Mira (Nek5000) vs. Summit CPU (Nek5000) and GPU (NekRS).  Timings are in
seconds for the averaged walltime per step for 400 timesteps over simulation
time interval [59.71, 59.78] with $\dt$= 1.9e-04 (CFL=0.32) at $Re_D=19000$ and
BDF2+EXT2 timerstepper.  

%% NEK: pebbles

Table~\ref{peb352k-strong} is the full core for the pebble bed reactor, which
