...
\includegraphics[width=0.93\textwidth]{../figs/pebble01.png}
 \caption{\label{fig:350k} High fidelity reactor simulations of turbulent flow performed on OLCF/Summit:
  ...
  $E$=98,782,067 elements of order $N=8$. We emphasize the difference in size enabled by Summit versus Mira.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
of full reactor cores at moderate Reynolds numbers cite \cite{Fang2021}.  An example
...
Nek5000 has been adropted by the Nuclear Regulatory Commission (NRC), the
...
The range of scales involved in the simulation of light water SMRs and SFRs, and in general of rodded cores, is shown in Figure~\ref{fig:smr1}: it can easily span 6 to 8 orders of magnitude from the micrometers involved in turbulent dissipation to meters, posing a formidable challenge.
...
important local/global interactions.

In Light Water SMRs, inter-bundle mixing
...
Further increase of the simulation fidelity including the explicit modeling of mixing vanes with Large Eddy Simulation will increase the accuracy
in the prediction in the flow distribution and heat transfer and produce high-quality datasets.  These unprecedented datasets will provide the basis needed to better understand the impact of three-dimensional effects, non-homogenous flow distribution and inter-assembly mixing and improve the performance of lower fidelity models typically used in modeling the core at the design stage. This increase in accuracy will translate in a reduction in uncertainty and a consequent reduction in margins and better economics. We emphasize that a complete picture of three-dimensional core-wide flows is nearly impossible to obtain with experiments alone, due to the scales involved, or even through operational data, due to the lack of adequate instrumentation in actual reactors.
% This is good, but I think it should be moved somewhere else
...
% Elia: I COMMENTED THIS OUT BECAUSE I DO NOT THINK IT IS A STRONG ARGUMENT. THE PROPOSAL HAS ITSELF A TIMELINE OF 3 years?
...
components has been demonstrated with LES \cite{merzari2017large}. For example, some landmark calculations conducted with Nek5000/RS are presented in
...
In this previous work we have modeled with RANS and momentum sources an SMR core comprised of 37 17x17 assemblies, inspired by the Nuscale core. Each assembly has an active fuel length of $2 m$ for which the the power distribution is obtained with a precursor Monte Carlo calculations.

We now propose to gradually increase the resolution of the simulations modeling an increasing number of assemblies with higher resolution using a zonal LES approach \cite{menter2012global}. The mixing vanes in the higher resolution assemblies will be modeled explicitly, and turbulence will be resolved using large eddy simulation. Finally, on Frontier, we propose to model the entire core with LES. The gradual approach will allow to make maximum use of available resources (i.e., an LES with mixing vanes is currently impossible on Summit) and it will also allow to identify the number of explicit assemblies needed to accurately account for the flow within a target assembly within the context of a zonal LES simulation.

The following tasks have been identified:
\vspace{-.15in}
\begin{enumerate}[label=\textbf{\Roman*}]
\item Develop full-core RANS mesh with one assembly explicitly modeled. Perform Mesh convergence studies for a single  LES bundles. Conduct CFD simulations and extract relevant information on flow spectra and mixing at the interface between resolved assembly and RANS model.
\item Develop full-core RANS mesh with nine assemblies explicitly modeled. Conduct CFD simulations and extract relevant information on flow spectra and mixing at the interface between resolved assembly and RANS model.
\item Develop full-core LES mesh with all assemblies explicitly modeled. Conduct CFD simulation. Extract relevant information on flow spectra and mixing at the interface between assemblies. Perform averaging and characterize flow distribution within the core.
\end{enumerate}
\vspace{-.15in}
...
the Zonal LES CFD simulations. Our group has extensive experience with modeling of mixing vanes \cite{Busco}; milestone I.A will develop guidelines for converged meshes for all subsequent simulations and deliver the first full core mesh with an assembly explicitly modeled, while milestone
