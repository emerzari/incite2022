...
\includegraphics[width=0.93\textwidth]{figs/pebble01.png}
 \caption{\label{fig:350k} High fidelity reactor simulations of turbulent flow performed on OLCF/Summit:
  ...
  %% MM: we might put project collaboration mention in text at one of earlier paragraphs, instead here?
...
NekRS is developed under ECP CEED project, targeting existing and exascale platforms at full scale.  
%% MM: above is quick intro nekrs/ceed but please feel free to adjust as your preferred way.
...
In this previous work we have modeled with RANS and momentum sources an SMR core comprised of 37 17x17 assemblies, inspired by the Nuscale core. Each assembly has an active fuel length of $2 m$ for which the power distribution is obtained with a precursor Monte Carlo calculations. The Reynolds number defined based on the rod diameter and the bulk velocity is $Re=50,000$. The Prandtl number is $Pr=1$.
