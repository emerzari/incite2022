...
In this previous work we have modeled with RANS and momentum sources an SMR core comprised of 37 17x17 assemblies, inspired by the Nuscale core. Each assembly has an active fuel length of $2 m$ for which the the power distribution is obtained with a precursor Monte Carlo calculations. The Reynolds number defined based on the rod diameter and the bulk velocity is $Re=50,000$. The Prandtl number is $Pr=1$.
