...
account for {\it interaction between global and local scales}. This
...
paths \cite{touran} -- without correction terms to account for the
...
motivating gap analysis -- reduced-geometry CFD models cannot account
for the interaction between core-wide physics and local thermal-fluids. By
developing more accurate lower fidelity models from turbulence-resolved
...
RANS \cite{conner2010cfd} and occasionally Unsteady RANS
 (URANS) remain the workhorses for analysis conducted in industry,
...
to make maximum use of available resources (i.e., a full-core LES with mixing vanes is
...
open-source prototypic reactor design \cite{abr}. The objectives of these simulations are to 1)~answer
outstanding questions about the nature of inter-assembly flow and heat transfer
and 2)~utilize the produced LES database to generate more accurate heat
transfer correlations. The major tasks have been identified as follows:
