...
%
\begin{wrapfigure}{rt}{2.8in} \begin{center}
{\setlength{\unitlength}{1.0in} \begin{picture}(2.900,0.600)(-.2,0)
  \includegraphics[scale=0.30]{figs/libp_kernels.png}
\end{picture}} \caption{\label{fig:roof} 
libParanumal Poisson-operator kernel development (iterations $K=1,\dots,10$) vs.
polynomial order, $N$   \cite{ceed_pb_paper_2020}.  \vspace*{-.2in} }
...
\newcolumntype{?}[1]{!{\vrule width #1}}
  ...
  \hline
  \hline
  System & Code      & Device & Node & Rank & R/N & $E$/R & $n$/Rank &  $t_{step} (s) $ &  Eff  \\
  \hline
  \hline
...
%%%%%%%%
  ...
  \end{tabular}
  \end{center}
\caption{\label{tab:mira}
Spacer-Grid performance on Mira (Nek5000), Summit CPU (Nek5000), and Summit GPU
(NekRS).  Timings are in seconds for the wall time per step, $t_{step}$
averaged over 100 timesteps with $\dt$= 6.00e-05 (CFL=1.706) at $Re_D=14000$
with pressure tol= 1.e-05 and velocity tol= 5.e-12.  }
\end{table*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
